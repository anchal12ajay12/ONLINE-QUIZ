...
try {
			...
			PreparedStatement ps = con.prepareStatement("select * from ( select * from addques where lang='java' order by dbms_random.value ) where rownum <= 5 ");
			...
			while(rs.next()){
				%>
				<% ++count; %>
				Ques <%=count %>:<%=rs.getString("ques").replace("\n","<br/>") %>
